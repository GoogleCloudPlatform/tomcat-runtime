...
/**
 * Implementation of the {@code org.apache.catalina.Manager} interface which uses a persistent
 * store to share sessions across nodes.
 *
 * <p>This manager should be used in conjunction with {@link KeyValuePersistentValve} and can be
 * used with {@link KeyValuePersistentStore}.<br/> Example configuration:</p>
 *
 * @param <S> The type of session specific to a storage system.
 */
public abstract class KeyValuePersistentManager<S extends StandardSession> extends
    ManagerBase implements StoreManager {

  private final Log log = LogFactory.getLog(this.getClass());

  /**
   * The store will be in charge of all the interactions with the storage.
   */
  protected Store store = null;

  @Override
  protected abstract S getNewSession();
  ...
  public Session findSession(String id) throws IOException {
    log.debug("KeyValuePersistentManager is loading session: " + id);
  ...
  /**
   * Remove the Session from the manager but not from the storage.
   *
  ...
  /**
   * Remove this Session from the active Sessions and the storage.
   *
   * @param session The session to remove.
   */
  @Override
